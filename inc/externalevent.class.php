...
class PluginPlanningExternalEvent extends CommonDBTM implements CalDAVCompatibleItemInterface {
...
JAVASCRIPT;
      ...
      echo "<td colspan='2'>";
      if ($canedit) {
         PluginPlanningPlanning::dropdownState("state", $this->fields["state"], true, [
            'rand' => $rand,
         ]);
      } else {
         echo PluginPlanningPlanning::getState($this->fields["state"]);
      }
      echo "</td>";
      echo "</tr>";
      ...
      echo "<td colspan='2'>";
      if ($canedit) {
         PluginPlanningEventCategory::dropdown([
            'value' => $this->fields['planningeventcategories_id'],
            'rand'  => $rand
         ]);
      } else {
         echo Dropdown::getDropdownName(
            PluginPlanningEventCategory::getTable(),
      ...
      echo "<td>";
      PluginPlanningPlanning::showAddEventClassicForm([
   ...
   public function getAsVCalendar() {
      ...
      $is_task = in_array($this->fields['state'], [PluginPlanningPlanning::DONE, PluginPlanningPlanning::TODO]);
   ...
   public function getInputFromVCalendar(VCalendar $vcalendar) {
      ...
      if ($vcomp instanceof VTodo && !array_key_exists('state', $input)) {
         ...
         $input['state'] = \PluginPlanningPlanning::TODO;
      }
