...
use atoum\atoum\mock\php\method;
use RRule\RRule;
 ...
 * Planning Class
 **/
class PluginPlanningPlanning extends CommonGLPI
{

   static $CONFIG = [
      'planning_types' => [
         'ChangeTask', 'ProblemTask', 'Reminder',
         'TicketTask', 'ProjectTask', 'PluginPlanningExternalEvent'
      ],
      'planning_begin' => '08:00:00', 'planning_end' => '20:00:00',
   ...
   static function getMenuContent()
   {
      ...
      if (PluginPlanningPlanning::canView()) {
         $menu = [
   ...
   static function getAdditionalMenuLinks()
   {
      ...
      if (PluginPlanningPlanning::canView()) {
         $title     = PluginPlanningPlanning::getTypeName(Session::getPluralNumber());
         $planning  = "<i class='fa far fa-calendar-alt pointer' title='$title'>
                        <span class='sr-only'>$title</span>
                       </i>";

         $links[$planning] = PluginPlanningPlanning::getSearchURL(false);
      }

      if (PluginPlanningExternalEvent::canView()) {
         $ext_title = PluginPlanningExternalEvent::getTypeName(Session::getPluralNumber());
         $external  = "<i class='fa fas fa-calendar-week pointer' title='$ext_title'>
                        <span class='sr-only'>$ext_title</span>
                       </i>";

         $links[$external] = PluginPlanningExternalEvent::getSearchURL(false);
      }
   ...
   static function getAdditionalMenuOptions()
   {
      if (PluginPlanningExternalEvent::canView()) {
         return [
            'external' => [
               'title' => PluginPlanningExternalEvent::getTypeName(Session::getPluralNumber()),
               'page'  => PluginPlanningExternalEvent::getSearchURL(false),
               'links' => [
   ...
   static function displayTabContentForItem(CommonGLPI $item, $tabnum = 1, $withtemplate = 0)
   {
      ...
      if ($item->getType() == __CLASS__) {
         switch ($tabnum) {
            case 1: // all
               PluginPlanningPlanning::showPlanning($_SESSION['glpiID']);
               break;
         }
      }
      return true;
   ...
   static function dropdownState($name, $value = '', $display = true, $options = [])
   {
      ...
      return Dropdown::showFromArray($name, $values, array_merge([
         'value'   => $value,
         'display' => $display
      ], $options));
   }
   ...
   static function checkAlreadyPlanned($users_id, $begin, $end, $except = [])
   {
      ...
      foreach (self::$CONFIG['planning_types'] as $itemtype) {
         $item = new $itemtype;
   ...
   static function checkAvailability($params = [])
   {
      ...
      $realbegin = $begin . " " . self::$CONFIG["planning_begin"];
      $realend   = $end . " " . self::$CONFIG["planning_end"];
      if (self::$CONFIG["planning_end"] == "24:00") {
      ...
      if (count($users) > 1) {
         ...
         $data = [0 => __('All')];
         $data += $users;
         Dropdown::showFromArray('limitto', $data, [
            'width' => '100%',
      ...
      if (count($displayuser)) {
         foreach ($displayuser as $who => $whoname) {
            ...
            $colnumber  = 1;
            $plan_begin = explode(":", self::$CONFIG["planning_begin"]);
            $plan_end   = explode(":", self::$CONFIG["planning_end"]);
   ...
   static function showPlanning($fullview = true)
   {
      ...
      Html::script('plugins/planning/js/planning.js');
   ...
   {
      global $CFG_GLPI;
      if (count(self::$CONFIG['planning_add_types']) == 1) {
         $params['itemtype'] = self::$CONFIG['planning_add_types'][0];
         self::showAddEventSubForm($params);
      } else {
         $rand = mt_rand();
         $select_options = [];
         foreach (self::$CONFIG['planning_add_types'] as $add_types) {
   ...
   static function showAddEventClassicForm($params = [])
   {
      ...
      $mintime = self::$CONFIG["planning_begin"];
      if (isset($params["begin"]) && !empty($params["begin"])) {
         $begin = $params["begin"];
      ...
      if ($display_dates) {
         ...
         Html::showDateTimeField("plan[begin]", [
            ...
            'maxdate'    => '',
            'mintime'    => $mintime,
            'maxtime'    => self::$CONFIG["planning_end"],
            'rand'       => $rand,
         ]);
         echo "</td></tr>";
      }
      ...
      $event_options = [
         ...
         'end'          => $end,
         'name'         => "plan[end]",
         'global_begin' => self::$CONFIG["planning_begin"],
         'global_end'   => self::$CONFIG["planning_end"]
      ];
   ...
   static function toggleFilter($options = [])
   {
      ...
      if (
         ...
         $_SESSION['glpi_plannings']['plannings'][$options['parent']]['users'][$options['name']]['display']
            = ($options['display'] === 'true');
      }
      self::savePlanningsInDB();
   }
   ...
   static function colorFilter($options = [])
   {
      ...
      if (
         ...
         $_SESSION['glpi_plannings']['plannings'][$options['parent']]['users'][$options['name']]['color'] = $options['color'];
      }
      self::savePlanningsInDB();
   }
   ...
   static function constructEventsArray($options = [])
   {
      ...
      $raw_events = [];
      $not_planned = [];
      foreach (self::$CONFIG['planning_types'] as $planning_type) {
      ...
      foreach ($raw_events as $event) {
         ...
         $users_id = (isset($event['users_id_tech']) && !empty($event['users_id_tech']) ?
            $event['users_id_tech'] :
            $event['users_id']);
         $content = PluginPlanningPlanning::displayPlanningItem($event, $users_id, 'in', false) ?: ($event['content'] ?? "");
         $tooltip = PluginPlanningPlanning::displayPlanningItem($event, $users_id, 'in', true) ?: ($event['tooltip'] ?? "");
         ...
         $new_event = [
            ...
            'color'       => (empty($event['color']) ?
               PluginPlanningPlanning::$palette_bg[$index_color] :
               $event['color']),
            'borderColor' => (empty($event['event_type_color']) ?
               self::getPaletteColor('ev', $event['itemtype']) :
               $event['event_type_color']),
            'textColor'   => PluginPlanningPlanning::$palette_fg[$index_color],
         ...
         if (isset($event['rrule']) && count($event['rrule'])) {
            ...
            $rset = PluginPlanningExternalEvent::getRsetFromRRuleField($rrule, $new_event['start']);
